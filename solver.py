...
#PUZZLE_FILE='/Users/zy/Downloads/IMG_3944.PNG'
#PUZZLE_FILE='unrecognized_shapes.png'
PUZZLE_FILE='/Users/zy/Downloads/IMG_3963.PNG'
import numpy as np
import pickle
import gzip
import sys
...
#    print len(patterns),'фэъ'
    ...
    for i in range(rows):
        row=[]
        for j in range(cols):
            roi=img[i*(bottom-top)/rows+20:(i+1)*(bottom-top)/rows-20,\
...
def print_board_masked(mask):
    for i in range(rows_count):
        for j in range(rows_count):
            print board[i+1,j+1] if (1<<rows_count*rows_count-((i*rows_count)+j)-1)&mask>0 else '.',
        print
...
def is_word_anywhere(word):
    ...
    for i in range(rows_count):
        for j in range(rows_count):
            result=result+is_word_in_position(i+1,j+1,word)
...
def list_posible_decompositions(words_with_masks):
    ...
    def list_decompositions_rec(possible_words_with_masks,already_used_words_masks,effective_mask,empty_cells,r):
        ...
        if r==2:
            print cnt[:3],cnt[3]
            cnt[:]=[0,0,999,already_used_words_masks[0][0]]
        if effective_mask == ((1<<(rows_count*rows_count))-1):
            solution = [w[0] for w in sorted(already_used_words_masks,key=len,reverse=True)]
            if len(solutions)==0 or len(solution)< min(map(len,solutions)):
                print 'Found solution in %d words'%len(solution)
            if not(solution in solutions):
                solutions.append(solution)
                cnt[2]=min(r,cnt[2])
        possible_mask=effective_mask
        for word_and_masks in possible_words_with_masks:
            for mask in word_and_masks[1]:
                possible_mask|=mask
        if possible_mask!=((1<<(rows_count*rows_count))-1):
            return
    ...
    #        print 'already:', repr(already_used_words_masks).decode("unicode-escape")    
        for idx,word_and_masks in enumerate(possible_words_with_masks):
            for mask in word_and_masks[1]:
                possible_now=[]
                for wm in possible_words_with_masks[(idx+1):]:
                    possible_masks_now=[m for m in wm[1] if (mask|effective_mask)&m==0]
                    if len(possible_masks_now)>0:
                        possible_now.append((wm[0],possible_masks_now))
                list_decompositions_rec(\
                        possible_now,\
                        already_used_words_masks+[(word_and_masks[0],mask)],\
                        effective_mask|mask,\
                        empty_cells-len(word_and_masks[0]),r+1)
    list_decompositions_rec(words_with_masks[:],[],0L,rows_count*rows_count,1)
...
def find_possible_words_in_position(i,j,possible_words):
    global possible_words_positions_pairs
    for possible_word in possible_words:
        possible_positions=is_word_in_position(i+1,j+1,possible_word)
...
def words_by_prefix(pref_indices):
    return words_hashed.get(''.join([board[i+1,j+1] for i,j in pref_indices]),[])
    ...
    for i in range(rows_count):
        for j in range(rows_count):
            for s in dominos_and_trominos:
                find_possible_words_in_position(i,j,\
                    words_by_prefix([(i+ii,j+jj) for ii,jj in s]))

 
    #print 'possible words/positions pairs',len(possible_words_positions_pairs)
    global possible_words_sorted_by_len
    possible_words_sorted_by_len=[]
    sorted_keys=sorted(possible_words_positions_pairs,reverse=True, key=lambda x:len(x))
    for key in sorted_keys:
        possible_words_sorted_by_len.append((key,possible_words_positions_pairs[key]))
...
def main():
    ...
    if not all(len(l)==rows_count for l in rows):
        ...
        return
    global board
    board=np.full((rows_count+3,rows_count+3),u' ',dtype=np.unicode)
    for i in range(rows_count):
        for j in range(rows_count):
